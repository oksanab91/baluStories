...
function travel_diaries_save_sidebar_layout( $post_id ) {
    global $travel_diaries_sidebar_layout, $post;
    ...
    foreach( $travel_diaries_sidebar_layout as $field ){
        ...
        $old = get_post_meta( $post_id, 'travel_diaries_sidebar_layout', true );
        $new = sanitize_text_field( $_POST['travel_diaries_sidebar_layout'] );
        if ( $new && $new != $old ) {
            update_post_meta( $post_id, 'travel_diaries_sidebar_layout', $new );
        } elseif ( '' == $new && $old ) {
            delete_post_meta( $post_id,'travel_diaries_sidebar_layout', $old );
        }
     } // end foreach
